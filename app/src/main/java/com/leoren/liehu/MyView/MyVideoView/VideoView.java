...
public class VideoView extends SurfaceView
        implements MediaController.MediaPlayerControl,Detector.OrientationChangeListener{
    ...
    private Detector mOrientationDetector;
    ...
    @Override
    public void onOrientationChanged(int screenOrientation, Detector.Direction direction) {
        if (!mAutoRotation) {
            return;
        }

        if (direction == Detector.Direction.PORTRAIT) {
            setFullscreen(false, ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
        } else if (direction == Detector.Direction.REVERSE_PORTRAIT) {
            setFullscreen(false, ActivityInfo.SCREEN_ORIENTATION_SENSOR_PORTRAIT);
        } else if (direction == Detector.Direction.LANDSCAPE) {
            setFullscreen(true, ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
        } else if (direction == Detector.Direction.REVERSE_LANDSCAPE) {
...
//                    }
    ...
    private void enableOrientationDetect() {
        if (mAutoRotation && mOrientationDetector == null) {
            mOrientationDetector = new Detector(mContext);
